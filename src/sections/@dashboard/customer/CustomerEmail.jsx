...
export default function CustomerEmail({ customer }) {
  ...
  useEffect(() => {
    const fetchEmail = async () => {
      try {
        const res = await customerApi.findMail(id)
        setEmails(res)
      } catch (e) {
  ...
  const handleChangeEmailList = (emails) => {
    setEmails(emails)
  }
  ...
  const filteredEmail = applySortFilter(
    emails,
    getComparator(order, orderBy),
    filterName
  )

  const isUserNotFound = filteredEmail.length === 0
    ...
    <Container>
      ...
      >
        <Button
          ...
          to='#'
          startIcon={<Iconify icon='ri:mail-send-fill' />}
        >
          Send Mail
        </Button>
      </Stack>
            ...
            <Table>
              ...
              <TableBody>
                {filteredEmail
                  ...
                  .map((row, index) => {
                    const { id, content, employeeId, createdAt } = row
