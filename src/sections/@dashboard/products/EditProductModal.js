...
const EditProductModal = ({ open, handleClose, product, setProduct }) => {
      ...
      <Paper elevation={5}>
        <Formik
          ...
          onSubmit={async (values) => {
            ...
            try {
              const res = await productApi.updateProduct(product.id, data)
              setProduct(res)
              handleClose()
            } catch (e) {
              if (e.statusCode === 403) {
                enqueueSnackbar('You must be ADMIN to update a product', {
                  variant: 'error',
                })
              } else {
